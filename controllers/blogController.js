...
exports.index = function (req, res, next) {
  Post.find({}, "title content date tags formatted_date")
    ...
    .exec(function (err, list_posts) {
      ...
      // for (let post in list_posts) {
      //   let unescaped = post.unescape()
      // }
      //Successful, so render
      res.render("index", {
        title: `Filip's Development Journal`,
...
exports.post_create_get = [
  ...
  function (req, res, next) {
    Tag.find({}, "name").exec(function (err, tag_list) {
      if (err) {
        return next(err);
      }
      //Successful, so render
...
exports.post_create_post = [
  ...
  (req, res, next) => {
    if (!(req.body.post_tags instanceof Array)) {
      if (typeof req.body.post_tags === "undefined") req.body.post_tags = [];
      else req.body.post_tags = new Array(req.body.post_tags);
    }
    next();
  },

  body("title", "Please input a title.")
    .trim()
    .isLength({ min: 1, max: 70 })
    .withMessage("title must be between 1 and 70 characters"),
    // .escape(),
  body("content", "Please input some content")
    .trim()
    .isLength({ min: 1 }),
    // .escape(),
...
exports.post_delete_post = function (req, res, next) {
  if (req.user) {
    ...
    res.render("post_details", { user: req.user, delete_error: true });
  }
};
